...
function Hero() {
  ...
  return (
    <motion.div
      ...
      className={`px-4 sm:px-20 xl:px-32 relative flex flex-col w-full justify-center items-center bg-cover bg-no-repeat min-h-screen gap-2`} style={{ backgroundImage: `url(${assets.gradientBackground})` }}>
      ...
      <div className="flex flex-wrap justify-center gap-4 mt-2 text-sm max-sm:text-xs">
        <button onClick={() => navigate('/ai')} className="bg-primary text-white px-10 py-3 rounded-lg hover:scale-102 active:scale-95 transition cursor-pointer">Explore AI Tools</button>
        <button className="border-1 border-black/70 px-10 py-3 rounded-lg hover:scale-102 active:scale-95 transition cursor-pointer">Watch demo</button>
      </div>
